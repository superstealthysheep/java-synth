...
public class AudioThread extends Thread {
    ...
    // these numbers are... pointers?
    private final int[] buffers = new int[BUFFER_COUNT];
    private final long device = alcOpenDevice(alcGetString(0, ALC_DEFAULT_DEVICE_SPECIFIER));
    private final long context = alcCreateContext(device, new int[1] /* no attributes */);
    ...
    AudioThread(Supplier<short[]> bufferSupplier) {
        ...
        for (int i = 0; i < BUFFER_COUNT; i++) {
            // "buffer samples"
            bufferSamples(new short[0]); // .. create "dummy" buffer (???)
        }
        alSourcePlay(source);
    ...
    public synchronized void run() {
        while (!closed) {
            ...
            for (int i = 0; i < processedBufs; i++) {
                ...
                alDeleteBuffers(alSourceUnqueueBuffers(source)); // out with the old
                buffers[bufferIndex] = alGenBuffers(); // in with the new: generate a new buffer
                bufferSamples(samples);
            }
            if (alGetSourcei(source, AL_SOURCE_STATE) != AL_PLAYING) {
                alSourcePlay(source);
            }
            catchInternalException(); // "just to be safe"
        }
    ...
    synchronized void triggerPlayback() {
        running = true;
        notify(); // exits wait() state
    }

    void close() {
        closed = true;
        triggerPlayback(); // must set `running` to true to break out of outer loop
    }

    private void bufferSamples(short[] samples) {
        int buf = buffers[bufferIndex++]; // pointer
